/**
 * Validate and convert file for Section Final
 * @param {IClientAPI} clientAPI
 */
export default async function ValidateAndConvertFileFinalB(clientAPI) {
  ...
  try {
    ...
    const photoSection = sectionedTable.getSection('SectionFinalUserInputImageB');
    const photoControl = photoSection?.getControl('SectionFinalTakePhotoB');
    const attachments = photoControl?.getValue() || [];

    const question = "Final Inspection B";
    ...
    await clientAPI.executeAction({
      Name: "/TRL_RH_SnorkelApp/Actions/AttachmentCreate.action",
      Properties: {
        Properties: {
          question: question,
    ...
    // ✅ Show next section
    // sectionedTable.getSection('SectionFinalMeasureName').setVisible(true);
    //  sectionedTable.getSection('SectionFinalQCMeasurements').setVisible(true);
