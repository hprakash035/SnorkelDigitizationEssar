 ...
 */
export default async function ValidateAndConvertFile162(clientAPI) {
    ...
    try {
        ...
        const photoSection = sectionedTable.getSection('Section162UserInputImage');
        const photoControl = photoSection?.getControl('Section162TakePhoto');
        const attachments = photoControl?.getValue();

        const question = sectionedTable
            .getSection('Section162Form')
            .getControl('Section162Question')
            .getValue();
        ...
        await clientAPI.executeAction({
            Name: "/TRL_RH_SnorkelApp/Actions/AttachmentCreate162.action",
            Properties: {
                Properties: {
                    question: question,
