 ...
 */
export default async function ValidateAndConvertFile131(clientAPI) {
    ...
    try {
        ...
        const photoSection = sectionedTable.getSection('Section131UserInputImage');
        const photoControl = photoSection?.getControl('Section131TakePhoto');
        const attachments = photoControl?.getValue();

        const question = sectionedTable
            .getSection('Section131Form')
            .getControl('Section131Question')
            .getValue();
        ...
        await clientAPI.executeAction({
            Name: "/TRL_RH_SnorkelApp/Actions/AttachmentCreate131.action",
            Properties: {
                Properties: {
                    question: question,
        ...
        // ✅ Show next section
        sectionedTable.getSection('Section132Form').setVisible(true);
