 ...
 */
export default async function ValidateAndConvertFile132(clientAPI) {
...
const binding = pageProxy.getBindingObject();
    ...
    try {
        ...
        const photoSection = sectionedTable.getSection('Section132UserInputImage123');
        const photoControl = photoSection?.getControl('Section132TakePhoto123');
        const attachments = photoControl?.getValue();

        const question = sectionedTable
            .getSection('Section132Form')
            .getControl('Section132Question')
            .getValue();
        ...
        await clientAPI.executeAction({
            Name: "/TRL_RH_SnorkelApp/Actions/AttachmentCreate132.action",
            Properties: {
                Properties: {
                    question: question,
